...
export default function PartnerForgotPasswordPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setError('');
        try {
            await api.post('/auth/forgot-password', { email, role: 'store_owner' });
            setSent(true);
        } catch (err: any) {
        ...
        <div className="min-h-screen gradient-white flex items-center justify-center px-4 py-12">
            <div className="w-full max-w-md">
                <div className="text-center mb-8 animate-fade-in">
                    ...
                    <h1 className="text-3xl font-bold mb-1"><span className="text-gradient">Reset Password</span></h1>
                ...
                <div className="glass-card rounded-2xl p-8 animate-scale-in">
                    {sent ? (
                        <div className="text-center py-4">
                            ...
                            <Link href="/partner/login" className="text-orange-600 font-semibold hover:text-orange-700 transition-colors">
                                ← Back to Login
                            </Link>
                        </div>
                    ) : (
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div>
                                ...
                                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                                    ...
                                    placeholder="you@store.com" required />
                            </div>
                    ...
                    <div className="mt-6 text-center">
                        <Link href="/partner/login" className="text-gray-500 hover:text-gray-700 text-sm transition-colors">← Back to Login</Link>
                    </div>
                </div>
            </div>
        </div>
